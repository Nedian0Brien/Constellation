...
2026-09-18 지도 호버·선택 모드 뒤로 논문 선택(URL `selected`)은 지도의 선택 모드(인용 선·이웃·활성 라벨·노드 둘레 버튼 셋)를 뜻하고, 논문 Dialog는 "노드 상세정보" 버튼(`useStore.detailOpen`, 세션)으로만 연다. 주제·분야 Dialog는 그대로다. run 안 인용 관계는 `/api/edges`(Tauri `edges`)로 한 번 받아 `views/map/edges.ts`의 CSR 인접 표로 둔다. `local=1`은 선택 논문의 2홉 로컬 그래프.
