...
// Dialog로 옮겼다. 주제·분야는 URL의 선택 상태가 열고, 닫으면(✕·Escape·바깥
// 클릭) 선택을 지운다. 논문은 지도의 선택 모드가 기본이라 "노드 상세정보"
// 버튼(`detailOpen`)으로만 열고, 닫아도 선택은 남는다. 논문이 주제·분야보다 우선한다.
export function InspectorDialog() {
  ...
  const open =
    (kind === "paper" &&
      detailOpen &&
      !!a.map.data?.id.includes(state.selected!)) ||
    (kind === "cluster" &&
  ...
  return (
    <Dialog
      ...
      onOpenChange={(open) => {
        if (open) return;
        if (kind === "paper") setDetailOpen(false);
        else update({ cluster: undefined, node: undefined });
      }}
    >
