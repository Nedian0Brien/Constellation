...
// NOTE(agent-chat-framework): speed(service tier) 선택을 더했다. effort 와
// 같은 규칙이다 — 모델이 `speeds` 를 주면 세그먼트가 그려지고, 선택은 모델을
// 바꿔도 남되 지원하지 않는 모델에서는 undefined 로 풀린다. 원본:
// vendor/assistant-ui/elements/model-selector.tsx
export type ModelSelectorSpeedOption = {
  id: string;
  name: string;
  description?: string;
};

/** The "standard" choice that clears the speed tier. Always rendered first. */
export const DEFAULT_SPEED_OPTION: ModelSelectorSpeedOption = {
  id: "",
  name: "Standard",
};
...
export type ModelOption = {
  ...
  /**
   * Speed tiers the model supports (e.g. a "Fast" mode). Omit for models
   * without configurable speed. The standard tier is implied and rendered
   * first; a selection of `undefined` means standard.
   */
  speeds?: readonly ModelSelectorSpeedOption[];
};
...
function getModelSpeeds(
  model: ModelOption | undefined,
): readonly ModelSelectorSpeedOption[] | undefined {
  if (!model?.speeds?.length) return undefined;
  return model.speeds;
}

function resolveSpeed(
  speeds: readonly ModelSelectorSpeedOption[] | undefined,
  speed: string | undefined,
): string | undefined {
  if (!speed) return undefined;
  return speeds?.some((s) => s.id === speed) ? speed : undefined;
}

/**
 * Returns the speed id if the given model supports it, otherwise undefined.
 * Like effort, speed is sticky across model switches.
 */
export function resolveModelSpeed(
  ...
  return resolveSpeed(
    getModelSpeeds(models.find((m) => m.id === modelId)),
    speed,
  );
}
...
type ModelSelectorContextValue = {
  ...
  /** The selected model's speed tiers, undefined when not configurable. */
  speeds: readonly ModelSelectorSpeedOption[] | undefined;
  /** Speed resolved against the selected model. `undefined` is standard. */
  speed: string | undefined;
  /** Pass `""` (or undefined) for standard. */
  setSpeed: (speed: string | undefined) => void;
...
/**
 * The selected model's speed tiers and the active selection. `speeds` is
 * undefined for models without a speed choice; `speed` undefined is standard.
 */
export function useModelSelectorSpeeds(): {
  speeds: readonly ModelSelectorSpeedOption[] | undefined;
  speed: string | undefined;
  setSpeed: (speed: string | undefined) => void;
} {
  const { speeds, speed, setSpeed } = useModelSelectorContext();
  return { speeds, speed, setSpeed };
}
...
export type ModelSelectorRootProps = {
  ...
  /** Speed tier id; `""` is standard. Controlled like `value`. */
  speed?: string;
  defaultSpeed?: string;
  /** Receives `""` when the user picks standard. */
...
function ModelSelectorRoot({
  ...
  // Standard is `""` in the controllable state so a controlled `speed` can
  // express it without flipping to uncontrolled (undefined) and going stale.
  const [speedState, setSpeedState] = useControllableState<string>({
    prop: speedProp,
    defaultProp: defaultSpeed ?? "",
    onChange: onSpeedChange,
  });
  const setSpeed = useCallback(
    (next: string | undefined) => setSpeedState(next ?? ""),
    [setSpeedState],
  );
  ...
  const speeds = getModelSpeeds(selectedModel);
  const activeSpeed = resolveSpeed(speeds, speedState || undefined);
...
export type ModelSelectorValueProps = {
  ...
  showEffort?: boolean;
  /** Show the active speed tier next to the model name. */
  showSpeed?: boolean;
  className?: string;
};
...
function ModelSelectorValue({
  ...
  const { selectedModel, efforts, effort, speeds, speed } =
...
/**
 * A labeled row of radio pills under the model list. Effort and Speed share
 * it; the keyboard handling keeps cmdk's Command root from stealing keys.
 */
function ModelSelectorSegment({
  slot,
  label,
  ariaLabel,
  options,
...
export type ModelSelectorEffortProps = SegmentProps;

function ModelSelectorEffort({ label = "Thinking", ...props }: ModelSelectorEffortProps) {
  const { efforts, effort, setEffort } = useModelSelectorEfforts();
  if (!efforts?.length) return null;
  return (
    <ModelSelectorSegment
      slot="model-selector-effort"
      label={label}
      ariaLabel={typeof label === "string" ? label : "Reasoning effort"}
      options={efforts}
      value={effort ?? ""}
...
export type ModelSelectorSpeedProps = SegmentProps;

/** Speed tier pills: Standard plus the model's `speeds`. Hidden without speeds. */
function ModelSelectorSpeed({ label = "Speed", ...props }: ModelSelectorSpeedProps) {
  const { speeds, speed, setSpeed } = useModelSelectorSpeeds();
  if (!speeds?.length) return null;
  return (
    <ModelSelectorSegment
      slot="model-selector-speed"
      label={label}
      ariaLabel={typeof label === "string" ? label : "Speed"}
      options={[DEFAULT_SPEED_OPTION, ...speeds]}
      value={speed ?? DEFAULT_SPEED_OPTION.id}
