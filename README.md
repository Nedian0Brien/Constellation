...
- **연구 지도**: 논문을 별로 배치하고 확대 수준에 따라 상위 분야·하위 분야·논문 제목을 표시한다. 영역 범위는 은은한 색 면, 분야 라벨은 중심의 흰색 글자다. 점에 1초 머물면 run 안 인용 관계(참조 파랑·피인용 빨강)와 이웃의 제목이 나타나고, 점을 클릭하면 선택 모드 — 노드 둘레의 버튼으로 상세, AI 질문, 2홉 로컬 그래프를 연다.
