...
- `revealZooms`는 절대 배율로 계산한다. 상대 배율로 하면 창 크기(`homeCamera`)가 바뀔 때 값이 어긋난다. 계산 바닥은 `Math.floor(home.zoom)` — 사이드바를 여닫아 지도 폭이 조금 바뀌어도 다시 계산하지 않는다(정수를 넘을 때만, 실측 60ms). 실데이터에서 바닥 `home.zoom`과 `floor(home.zoom)`의 표시 결과가 10,604편 모두 같았다.
