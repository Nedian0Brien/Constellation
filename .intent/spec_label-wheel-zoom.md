...
- [ ] 켜진 `.region-name` 위에서 `pointerdown` 뒤 4px 넘게 움직이면 지도의 `target`이 따라온다. 라벨 밖에서 같은 만큼 끌었을 때와 같은 양이다(같은 배율에서 화면 픽셀 ↔ 지도 좌표 변환이 같다).
- [ ] 끄는 동안 마우스가 라벨을 벗어나도 놓을 때까지 이어진다(포인터 캡처).
- [ ] 끈 뒤에 떼면 그 라벨의 `onClick`이 불리지 않는다. 4px 안에서 떼면 불린다.
- [ ] 끄는 동안 커서는 `grabbing`, 그 밖에는 `grab`(라벨 밖의 지도와 같다).
