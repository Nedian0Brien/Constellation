...
구현 중 더한 것(두 번째 지시): 버튼 각도를 위쪽 호(−150°·−90°·−30°)로 — 아래 두 버튼이 노드 제목을 가렸다. 로컬 그래프를 끄면 켜기 전 카메라로 돌아간다(E2E가 그 뒤 제목 단계를 기대한다). 활성 라벨의 장애물은 이 칸의 지도 제목 전부(켜짐 여부 무관). README·ARCHITECTURE에 선택 모드와 edges를 적었다.
