...
- [ ] `ResizablePanelGroup`, `react-resizable-panels` 의존, `components/ui/resizable.tsx`, `AppShell`의 `mobile ? … : …` 토글 분기와 `matchMedia` 상태, `index.css`의 `.app-sidebar*`·`.inspector-*`·`.detail`(266행 레거시와 1410행 `!important` 덮어쓰기 모두)·`.close`·`.tag`·`.meta-row`·`.corpus-links`·`.cl-*`·`.sidebar-note`·`.cluster-count`·`.topic-dot` 셀렉터를 제거한다. 다른 파일이 쓰는 셀렉터(`.topic`, `.eyebrow`, `.spark*`)는 남긴다.
