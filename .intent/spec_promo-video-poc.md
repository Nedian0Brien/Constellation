...
- [ ] `MapDive` 컴포지션 5초(150프레임)의 장면 순서:
  ...
  - 초점 논문은 컴포지션 props `paperId`로 정한다. 그 논문이 속한 상위·하위 분야를 거슬러 찾아 그 순서로 들어간다. `paperId`가 없으면 가장 큰 상위 분야 → 그 안의 가장 큰 하위 분야 → 피인용수 최대 논문 순으로 고른다.
  - 기본값은 *Open-Ended Instructable Embodied Agents with Memory-Augmented LLMs*(2023, `openalex:W4389523655`)다(사용자 결정, 2026-09-24). 자동 규칙은 *Gene Ontology*(2000)를 골라 코퍼스 주제와 맞지 않았다. 사용자는 피지컬 AI 논문을 원했고 피지컬 AI 코퍼스를 새로 만들기로 했다. 그 전까지는 현재 코퍼스에서 체화 AI에 가장 가까운 이 논문을 쓴다.
  - 앱의 강조 라벨(강조 노드 제목 전문 타이핑, 이웃 라벨)은 이번에 옮기지 않는다. 인용선, 이웃 점, 나머지 점 흐리기까지만 옮긴다.
