...
fn filter_matches_list_and_keeps_unknown_year() {
    ...
    assert_eq!(
        queries::matches(&f.db, flt.clone()).unwrap().ids,
        ["1", "2"]
    );
...
fn stable_paging_nulls_last_and_run_isolation() {
    ...
    assert_eq!(
        queries::matches(&f.db, filter("b", "", None, None))
            .unwrap()
            .ids,
        ["4"]
    );
...
fn literal_search_and_empty_results() {
    let f = populate();
    assert_eq!(
        queries::matches(&f.db, filter("a", "0%", None, None))
            .unwrap()
            .ids,
        ["5"]
    );
    assert_eq!(
        queries::matches(&f.db, filter("a", "zzzz", None, None))
            .unwrap()
            .total,
        0
    );
...
fn invalid_filters_are_422_and_missing_run_is_404() {
    ...
    assert_eq!(
        status(queries::matches(&f.db, filter("a", "x", None, None)).unwrap_err()),
        422
    );
    assert_eq!(
        status(queries::matches(&f.db, filter("a", "", Some(2022), Some(2020))).unwrap_err()),
        422
    );
    assert_eq!(
        status(
...
fn runs_health_clusters_and_detail() {
    ...
    assert_eq!(
        queries::cluster_detail(&f.db, "a", 9).unwrap_err().status,
        404
    );
...
fn lineage_main_path_and_neighbourhood() {
    ...
    assert_eq!(
        queries::lineage(&f.db, "a", None, 9, 240)
            .unwrap_err()
            .status,
        422
    );
    assert_eq!(
        queries::lineage(&f.db, "b", None, 2, 240)
            .unwrap_err()
            .status,
        404
    );
